module.exports = {
   ...
   */
    createExe: function(name, dir, callback) {
        ...
        var exeOrigin = path.join(__dirname, '../bin/winsw/winsw.exe');
        var cfgOrigin = path.join(__dirname, '../bin/winsw/winsw.exe.config');
        var exeDest = path.join(dir, name.replace(/[^\w]/gi, '').toLowerCase() + '.exe');
        var cfgDest = path.join(dir, name.replace(/[^\w]/gi, '').toLowerCase() + '.exe.config');

        var exeData = fs.readFileSync(exeOrigin, { encoding: 'binary' });
        var cfgData = fs.readFileSync(cfgOrigin, { encoding: 'binary' });
